...
def remove_duplicates_and_short_entries(input_file, output_file, min_length=4):
    try:
        ...
        filtered_lines = set(line.strip() for line in lines if '-' not in line and "'" not in line and '.' not in line and '�' not in line and len(line.strip()) >= min_length and len(line.strip()) <= 15)
